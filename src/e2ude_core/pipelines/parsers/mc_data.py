...
def parse_mcdata(file_path: Path) -> Dict[Type[Base], pd.DataFrame]:
    ...
    for str_key, model_tuple in parser_map.items():
        ...
        bad_row = next((row for row in raw_rows if len(row) != len(columns)), None)
        if bad_row is not None:
            raise ValueError(
                f"Malformed {str_key} row at source line {bad_row[0]}: "
                f"expected {len(columns)} values, got {len(bad_row)}"
            )
