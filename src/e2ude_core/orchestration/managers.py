...
# ... (JobManager and JobControl classes remain unchanged) ...
...
class SessionManager:
    ...
    def check_for_completed_job(
        self,
        pipeline_id: str,  # Unused in Artifact check, but kept for API compat
        hash_id: int,
        target_name: str,
        required_version: int,
    ) -> bool:
        """
        Checks the ArtifactManifest to see if valid data already exists.
        Now completely decoupled from the ProcessingJob history log.
        """
        session = self.Session()
        try:
            # Simple, direct lookup. No joins.
            # "Do we have an artifact for this hash + table with version >= X?"
            stmt = select(ArtifactManifest.handler_version).where(
                ArtifactManifest.hash_id == hash_id,
                ArtifactManifest.target_table == target_name,
            )
            ...
            return existing_version >= required_version
    ...
    def finalize_session(self):
        ...
        try:
            ...
            # Count errors to determine session status
            failed_jobs = (
