...
def get_folder_work_delta(
    ...
    with eng.connect() as conn:
        ...
        actual_work = {
            (row.hash_id, row.target_table) for row in conn.execute(actual_stmt)
        }
...
def get_folder_states_bulk(
    ...
    with eng.connect() as conn:
        ...
        for i in range(0, len(folders_to_check), SQL_BATCH_SIZE):
            ...
            file_query = sa.select(
                FileMetadata.folder_id, FileMetadata.hash_id, FileMetadata.file_type
        ...
        for i in range(0, len(hash_id_list), SQL_BATCH_SIZE):
            ...
            art_query = sa.select(
                ArtifactManifest.hash_id, ArtifactManifest.target_table
