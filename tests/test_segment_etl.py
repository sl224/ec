...
def get_segment_df(lines):
    ...
    rows = []
    for line in lines:
        tokens = line.split(",", maxsplit=len(COLUMNS))
        tokens.pop()
        rows.append(tokens)
...
def get_zip_paths(eng):
    query = """
    select distinct fo.FolderID, FolderPath from E2D_METADATA.FOLDER as fo 
    left join E2D_METADATA.FILES as fi on fi.FolderID = fo.FolderID
    where fi.FolderID is NULL
    """
    with eng.connect() as conn:
        return [(fid, Path(fp)) for fid, fp in conn.execute(sa.text(query)).fetchall()]


class SegmentETL(unittest.TestCase):
...
def segment_suite():
    ...
    suite.addTest(SegmentETL("test_run_file_etl"))
    return suite
